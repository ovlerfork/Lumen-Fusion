...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  void passthrough(std::shared_ptr<input_t> &input, std::vector<std::uint8_t> &&input_data) {
    ...
    // allocation, deallocation, and back button logic go through the task pool.
    if (input_data.size() >= sizeof(NV_MULTI_CONTROLLER_PACKET)) {
      auto header = (PNV_INPUT_HEADER) input_data.data();
      if (util::endian::little(header->magic) == MULTI_CONTROLLER_MAGIC_GEN5) {
        auto packet = (PNV_MULTI_CONTROLLER_PACKET) input_data.data();
        auto controller = packet->controllerNumber;

        if (controller >= 0 && controller < (int) input->gamepads.size()) {
          auto &gamepad = input->gamepads[controller];
          // Only fast-path if gamepad is allocated AND packet is a normal state update
          // (not an allocation or deallocation). Fall through to task pool otherwise.
          if (gamepad.id >= 0 && (packet->activeGamepadMask & (1 << controller))) {
            std::uint16_t bf = packet->buttonFlags;
            std::uint32_t bf2 = packet->buttonFlags2;
            platf::gamepad_state_t gamepad_state {
              bf | (bf2 << 16),
